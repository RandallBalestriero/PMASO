...
XX = x_train[pp]+randn(len(pp),1,28,28)*0.15
YY = y_train[pp]
...
if(modeln==0):
	...
	layers1 = [InputLayer(input_shape)]
	layers1.append(ConvLayer(layers1[-1],stride=1,Ic=3,Jc=3,K=8,R=2,nonlinearity=None,sparsity_prior=0.))
...
LOSSES  = train_model(model1,rcoeff=40,CPT=100,random=1,fineloss=1)
reconstruction = model1.reconstruct()[:150]
samplesclass1  = [model1.sampleclass(int(k<4),k) for k in xrange(10)]
