...
class DenseLayer:
	def __init__(self,input_layer,K,R,sparsity_prior = 0,leakiness=None,sigma='local',p_drop=0,residual=0,alpha=0.95,update_b=True):
		...
		self.drop_   = tf.Variable(tf.ones((self.K,2,self.bs))*tf.reshape(tf.one_hot(1,2),[1,2,1]))
		    ...
		    self.W_  = tf.Variable(tf.random_normal((K,R,self.D_in),float32(0),float32(0.0001)))
                    self.W   = self.W_
                else:
...
#		    tf.Tensor.set_shape(K,[self.bs,self.K,self.R])
	...
	def likelihood(self,batch=None):
		...
		if(self.p_drop>0):
				...
				-tf.reduce_sum(self.drop_*tf.log(self.drop_+eps)))/float32(self.bs)+float32(self.D_in/2.0)*tf.log(2*PI_CONST)
                    ...
                    return self.likelihood(0)+(-tf.reduce_sum(self.p_*tf.log(self.p_+eps))+tf.reduce_sum(tf.log(self.v2_))*float32(0.5))/float32(self.bs)+float32(self.D_in/2.0)*tf.log(2*PI_CONST)
                                                            ################### E STEP VARIABLES
        def update_v2(self):
                scalor = tf.reduce_min(self.next_layer.sigmas2_)
		a40 = self.drop_[:,1]*tf.einsum('knr,krd,d->kn',self.p_,tf.square(self.W),scalor/self.sigmas2_)+scalor/tf.expand_dims(self.next_layer.sigmas2_,-1)
...
#			if(self.residual): reconstruction = self.input_-self.b-self.backwardmk(self.k_,with_m=1,m=new_m);tf.Tensor.set_shape(reconstruction,[self.bs,self.D_in]) # (N D)
        ...
        def update_sigma(self):
                ...
                if(self.sigma_opt=='local'):     return tf.assign(self.sigmas2_,tf.clip_by_value(value,0.00000001,10))
                elif(self.sigma_opt=='global'):  return tf.assign(self.sigmas2_,tf.fill([self.D_in],tf.reduce_sum(value)/self.D_in))
        ...
        def update_pi(self):
                return tf.assign(self.pi,self.S7/tf.reduce_sum(self.S7,axis=1,keepdims=True))
        def update_Wk(self):
                k = self.k_
...
class ConvLayer:
	def __init__(self,input_layer,K,Ic,Jc,R,sparsity_prior = 0,leakiness=None,sigma='local',alpha=0.5):
		...
		self.D_in              = prod(self.input_shape[1:])
                ...
                self.pi_                 = PI_CONST
                self.W_  = tf.Variable(tf.truncated_normal((self.K,self.Ic,self.Jc,self.C),0,1))# (K,R,Ic,Jc,C) always D_in last
		...
		self.drop_   = tf.Variable(tf.ones((K,2,self.I,self.J,self.bs))*tf.reshape(tf.one_hot(1,2),(1,2,1,1,1))) # (K 2 I J N)
                ...
                # STATISTICS
                self.S1 = tf.Variable(tf.zeros(self.input_shape[1:]))
                self.S2 = tf.Variable(tf.zeros(self.input_shape[1:]))
                ...
                self.S7 = tf.Variable(tf.zeros(self.input_shape[1:]))
        ...
        def filter_corr(self,A,B):
            ...
            return tf.transpose(tf.reshape(tf.einsum('nijabc,kabc->kijn',patches,B),[self.K,2*self.Ic-1,2*self.Jc-1,self.K,self.I,self.J]),[3,4,5,0,1,2])#(K I J K' I' J')
        def update_S(self):
            S1 = tf.reduce_sum(tf.square(self.input_layer.m),0)/self.bs
            S2 = tf.reduce_sum(self.input_layer.v2,0)/self.bs
            ...
            E  = tf.sparse_to_dense(indices,(self.K,self.I,self.J,self.K,2*self.Ic-1,2*self.Jc-1),float32(1))
            S5 = tf.einsum('kijn,nijabc->kijcab',self.m_*self.p_[:,:,:,0],mp_patches)*(1-E)/self.bs
            S6 = tf.reduce_sum(self.m_*self.p_[:,:,:,0],3)/self.bs
            S7 = tf.reduce_sum(self.input_layer.m,0)/self.bs
	...
	def sample(self,M,K=None,sigma=1):
        ...
        def likelihood(self,batch=0):
                if(batch==0):
                    ...
                    a3  = tf.einsum('kijn,kabc,ijabc->',tf.square(self.m_*self.p_[:,:,:,0]),tf.square(self.W_),1/self.sigmas2_patch_)*0.5/self.bs
                    a4  = -tf.einsum('kijn,kabc,ijabc->',(tf.square(self.m_)+self.v2_)*self.p_[:,:,:,0],tf.square(self.W_),1/self.sigmas2_patch_)*0.5/self.bs
		    ...
		    k3  = -float32(self.D_in/2.0)*tf.log(2*PI_CONST)
                    ...
                    k0 = -float32(self.D_in/2.0)*tf.log(2*PI_CONST)-tf.reduce_sum(tf.log(self.sigmas2_))/2
                    k1 = tf.reduce_sum((-self.S1-self.S2)/self.sigmas2_)*0.5
                    k2 = tf.reduce_sum(self.S3*tf.einsum('kabc,ijabc->kijabc',self.W_,1/self.sigmas2_patch_))
                    k3 = -tf.reduce_sum(self.S4*tf.einsum('kabc,ijabc->kij',tf.square(self.W_),1/self.sigmas2_patch_))*0.5
                    filters = self.filter_corr(self.W_,self.W_) # (kij k'i'j')
                    k4 = -tf.reduce_sum(self.S5*filters)*0.5
#                    k4 = tf.einsum('kij,kabc,ijabc->',tf.square(self.S6),tf.square(self.W_),0.5/self.sigmas2_patch_)-tf.reduce_sum(tf.square(tf.gradients(self.sigmas2_patch_,self.sigmas2_,tf.einsum('kij,kabc->ijabc',self.S6,self.W_))[0])/self.sigmas2_)*0.5
                    k5 = -tf.reduce_sum(self.S6*tf.einsum('kabc,ijabc->kij',self.W_,self.b_patch_/self.sigmas2_patch_))
                    k6 = -tf.reduce_sum(tf.square(self.b_)/self.sigmas2_)*0.5
                    k7 = tf.reduce_sum(self.S7*self.b_/self.sigmas2_)
                    k8 = tf.reduce_sum(self.S8*tf.log(self.pi))
                    return k0+k1+k2+k3+k4+k5+k6+k7+k8
        def KL(self):
                return self.likelihood(0)+(-tf.reduce_sum(self.p_*tf.log(self.p_+eps))+float32(0.5)*tf.reduce_sum(tf.log(self.v2_)))/float32(self.bs)+float32(self.D_in/2.0)*tf.log(2*PI_CONST)
	def update_dropout(self):
        ...
        def update_v2(self):# DONE
                ...
                scalor = tf.reduce_min(v_value)
                a4 = tf.einsum('kijn,kabc,ijabc->kijn',self.p_[:,:,:,0],tf.square(self.W_),scalor/self.sigmas2_patch_) # (K I J N)
                return tf.assign(self.v2_,scalor/(tf.transpose(scalor/v_value,[3,1,2,0])+a4))
	def update_m(self,mp_opt=0):
		reconstruction = (self.input-self.b-self.deconv(None,1,0))/self.sigmas2
		### UPDATE M
		if(mp_opt==0):
                        forward_m   = tf.einsum('nija,a->nij',self.extract_patch(reconstruction,with_reshape=0)[:,self.i_::self.ratio,self.j_::self.ratio]*tf.expand_dims(self.p[:,self.i_::self.ratio,self.j_::self.ratio,self.k_,0],-1),tf.reshape(self.W_[self.k_],[-1]))# (N I' J')
	                if(isinstance(self.next_layer,ConvLayer) or isinstance(self.next_layer,PoolLayer)): 
                            back = (self.next_layer.deconv()[:,self.i_::self.ratio,self.j_::self.ratio,self.k_]\
                                    +self.next_layer.b[:,self.i_::self.ratio,self.j_::self.ratio,self.k_])\
                                    /self.next_layer.sigmas2[:,self.i_::self.ratio,self.j_::self.ratio,self.k_]
			else:       
                            back = (self.next_layer.backward(0)[:,self.i_::self.ratio,self.j_::self.ratio,self.k_]\
                                    +self.next_layer.reshape_b[:,self.i_::self.ratio,self.j_::self.ratio,self.k_])/\
                                    self.next_layer.reshape_sigmas2[:,self.i_::self.ratio,self.j_::self.ratio,self.k_]
			...
			return new_m
                        ...
                        forward = tf.einsum('nija,a->nij',self.extract_patch(reconstruction,with_reshape=0)[:,self.i_::self.ratio,self.j_::self.ratio]*tf.expand_dims(self.m[:,self.i_::self.ratio,self.j_::self.ratio,self.k_],-1),tf.reshape(self.W_[self.k_],[-1]))# (N I' J')
	                ...
	                value   = forward-m2v2+tf.log(self.pi[self.k_,0]/self.pi[self.k_,1])# (N I' J')
                        update_value = tf.nn.sigmoid(value)
	                new_p        = tf.scatter_nd_update(self.p_,self.indices_,tf.transpose(tf.reshape(tf.stack([update_value,1-update_value]),(self.R,self.bs,-1)),[2,0,1]))
			return new_p
        def update_Wk(self):
#                return []
#                filters = self.filter_corr(tf.sparse_to_dense([[self.k_,self.i_,self.j_,ii] for ii in xrange(self.C)],[self.K,self.Ic,self.Jc,self.C],float32(1)),self.W_) # (kij k'i'j')
#                value  = tf.reduce_sum(self.S5*filters)
##
#                filters   = tf.einsum('kabc,ijc->kijabc',tf.pad(self.W_[:,::-1,::-1],[[0,0],[self.i_,self.Ic-self.i_-1],[self.j_,self.Jc-self.j_-1],[0,0]]),1/self.sigmas2_patch_[:,:,self.i_,self.j_,:])#(kijabc
#                filters2  = tf.pad(tf.einsum('kabc,ijabc->kijabc',self.W_,1/self.sigmas2_patch_),[[0,0],[0,0],[0,0],[self.Ic-self.i_-1,self.i_],[self.Jc-self.j_-1,self.j_],[0,0]])#(kijabc
#                value     = tf.einsum('abcdef,abcdefz->z',self.S5,tf.expand_dims(filters,3)+tf.expand_dims(tf.transpose(filters2,[1,2,0,3,4,5]),0))*0.5#(c)
        ...
        def update_BV(self):
                return tf.assign(self.b_,self.S7-tf.gradients(self.sigmas2_patch_,self.sigmas2_,tf.einsum('kij,kabc->ijabc',self.S6,self.W_))[0])
        def update_sigma(self):
                v1 = self.S1+self.S2-2*self.b_*self.S7+tf.square(self.b_)
                ...
                patches = tf.reshape(tf.extract_image_patches(sigma_W,(1,self.Ic,self.Jc,1),(1,1,1,1),(1,1,1,1),"VALID"),(self.K*self.I*self.J,2*self.Ic-1,2*self.Jc-1,self.Ic,self.Jc,self.C))
                scalar = tf.reduce_sum(self.S5* tf.transpose(tf.reshape(tf.einsum('nijabc,kabc->kijn',patches,self.W_),[self.K,2*self.Ic-1,2*self.Jc-1,self.I,self.J,self.K]),[5,3,4,0,1,2]))#(K I J K' I' J')
...
class PoolLayer:
	def __init__(self,input_layer,Ic,Jc,sigma='local',alpha=0.5):
		...
		self.D_in              = prod(self.input_shape[1:])
                ...
                self.W                 = self.W_
		# WE DEFINE THE PARAMETERS
                self.pi      = tf.Variable(mysoftmax(tf.random_normal((R,))))
		self.sigmas2_= tf.Variable(tf.ones((self.Iin,self.Jin,self.C)))
		self.sigmas2 = tf.expand_dims(self.sigmas2_,0)
                self.sigmas2_patch_= self.extract_patch(self.sigmas2,with_n=0)
		self.sigmas2_patch = tf.expand_dims(self.sigmas2_patch_,0)
        ...
        def update_S(self):
            ...
            S5 = tf.reduce_sum(self.p_,[0,1,2,4])/self.bs
            return tf.group(tf.assign(self.S1,self.alpha*S1+(1-self.alpha)*self.S1),tf.assign(self.S2,self.alpha*S2+(1-self.alpha)*self.S2),
                        tf.assign(self.S3,self.alpha*S3+(1-self.alpha)*self.S3),tf.assign(self.S4,self.alpha*S4+(1-self.alpha)*self.S4),
                        tf.assign(self.S5,self.alpha*S5+(1-self.alpha)*self.S5))
	def extract_patch(self,u,with_n=1,with_reshape=1):
...
#                                           ---- BACKWARD OPERATOR ---- 
	...
	def sample(self,M,K=None,sigma=1):
        ...
        def likelihood(self,batch=0):
                if(batch==0):
                    a1  = -tf.einsum('nijc,ijc->',tf.square(self.input-self.deconv()-self.b),1/(2*self.sigmas2_*self.bs))
                    a2  = -tf.einsum('cijn,ijc->',self.input_layer.v2_,1/(2*self.sigmas2_*self.bs))
                    a3  = tf.reduce_sum(tf.square(tf.einsum('kijn,kijrn,rab->ijabk',self.m_,self.p_,self.W_))/self.sigmas2_patch_)*0.5/self.bs
                    a4  = -tf.einsum('kijrn,rab,ijabc->',tf.expand_dims(tf.square(self.m_)+self.v2_,3)*self.p_,tf.square(self.W_),1/self.sigmas2_patch_)*0.5/self.bs
                    k1  = -tf.reduce_sum(tf.log(self.sigmas2_))/2
                    k2  = tf.einsum('r,kijrn->',tf.log(self.pi),self.p_)/float32(self.bs)
		    k3  = -float32(self.D_in/2.0)*tf.log(2*PI_CONST)
                    return k1+k2+k3+(a1+a2+a3+a4)
                else:
                    ...
                    k4 = tf.reduce_sum(self.S5*tf.log(self.pi))
	...
	def update_dropout(self):
        ...
        def update_v2(self):# DONE
                ...
                scalor = tf.reduce_min(v_value)
                a4 = tf.einsum('kijrn,rab,ijabc->kijn',self.p_,tf.square(self.W_),scalor/self.sigmas2_patch_) # (K I J N)
                return tf.assign(self.v2_,scalor/(tf.transpose(scalor/v_value,[3,1,2,0])+a4))
	def update_m(self,mp_opt=0):
		### UPDATE M
		if(mp_opt==0):
                        forward_m   = tf.einsum('nijabc,cijrn,rab->nijc',self.extract_patch(self.input),self.p_,self.W_)# (N I J K)
			...
			update_value_m = (forward_m+back)*self.v2 # (N I J K)
	                new_m = tf.assign(self.m_,tf.transpose(update_value_m,[3,1,2,0]))
			return new_m
                else:
                        forward = tf.einsum('nijabc,cijn,rab->cijrn',self.extract_patch(self.input),self.m_,self.W_)# (K I J R N)
	                ...
	                value   = forward-m2v2+myexpand(tf.log(self.pi),[0,0,0,-1])# (N I' J')
                        update_value = tf.nn.softmax(value,axis=3)
...
#                v5 = tf.gradients(self.sigmas2_patch_,self.sigmas2_,tf.einsum('kij,kabc,ijabc->ijabc',self.S6,self.W_,self.b_patch_))[0]
                value = v1+v2+v3
                if(self.sigma_opt=='local'):
                    return tf.assign(self.sigmas2_,value)
                elif(self.sigma_opt=='channel'):
...
class CategoricalLastLayer:
        def __init__(self,input_layer,R,sparsity_prior=0,sigma='local',alpha=0.5,update_b=True):
                self.update_b          = update_b
		...
		# PARAMETERS
                self.sigmas2_= tf.Variable(tf.ones(self.D_in),trainable=False)
                self.sigmas2 = tf.expand_dims(self.sigmas2_,0)
		self.W       = tf.Variable(tf.random_normal((R,self.D_in),float32(0),0.1))
		self.pi      = tf.Variable(tf.ones(R)/R) # (1 R)
                self.b_      = tf.Variable(tf.zeros(self.D_in))
                self.b       = tf.expand_dims(self.b_,0)
		# VI PARAMETERS
	...
	def likelihood(self,batch=None):
                if(batch==0):
                    ...
                    return -k1-k2+k3+k4
        def KL(self):
                return self.likelihood(0)-tf.reduce_sum(self.p*tf.log(self.p+eps))/float32(self.bs)
                                        ################### E STEP UPDATES
	def update_v2(self):
		return []
        def update_rho(self):
        ...
        def update_sigma(self):
                ...
                if(self.sigma_opt=='local'):    return tf.assign(self.sigmas2_,tf.clip_by_value(value,0.0000000001,10000))
                elif(self.sigma_opt=='global'): return tf.assign(self.sigmas2_,tf.fill([self.D_in],tf.reduce_sum(value)/self.D_in))
