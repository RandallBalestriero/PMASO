...
layers1.append(DenseLayer(layers1[-1],K=32,R=2,nonlinearity=None,sparsity_prior=0.00,sigma=sigmass))
for l in xrange(n_layers):
	layers1.append(DenseLayer(layers1[-1],K=32,R=2,nonlinearity=None,sparsity_prior=0.00,sigma=sigmass))
layers1.append(FinalLayer(layers1[-1],10,sparsity_prior=0.00,sigma=sigmas))
...
LOSSES  = train_layer_model(model1,rcoeff_schedule=schedule(0.000000000001,'linear'),CPT=2,random=0,fineloss=0,verbose=0)
...
f=open(SAVE_DIR+'exp_resnet_'+str(n_layers)+'_'+str(residual)+'_'+str(sigmass)+'.pkl','wb')
