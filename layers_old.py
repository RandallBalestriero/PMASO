...
class DenseLayer:
	def __init__(self,input_layer,K,R,sparsity_prior = 0,nonlinearity='relu',b='local',sigma='local',U='local',batch_norm=BatchNorm(1,1),learn_pi=1,p_drop=0):
		...
		# BN PARAMETERS
                self.D_      = tf.Variable(tf.ones(self.D_in))
                self.D       = tf.expand_dims(self.D_,0)
                ...
                # PARAMETERS
		self.drop_   = tf.Variable(tf.ones((self.K,2,self.bs))/2)
#		self.drop    = tf.transpose(self.drop_,[2,0,1])
		...
		# RESHAPE IF NEEDED
                if(len(self.input_shape)>2):
                        ...
                        self.reshape_D        = tf.expand_dims(tf.reshape(self.D_,self.input_shape[1:]),0)
...
#                                           ---- BACKWARD OPERATOR ---- 
	...
	def backwardmk(self,k):
                return tf.tensordot(self.p_*tf.expand_dims(self.m_*self.drop_[:,1]*tf.expand_dims(1-tf.one_hot(k,self.K),1),-1),self.W,[[0,2],[0,1]])
	def sample(self,M,K=None,sigma=1,deterministic=False):
		if(deterministic):
                    return tf.reshape((self.backward(1)+self.b)*self.U*self.D+self.mu,self.input_shape)
		...
		if(self.p_drop>0):
		    ...
		    K = tf.transpose(tf.reshape(tf.one_hot(tf.multinomial(tf.log(self.pi),self.bs),self.R),(self.K,self.bs,self.R)),[1,0,2])
                return tf.reshape(tf.cast((tf.tensordot(tf.expand_dims(M*dropout,-1)*K,self.W,[[1,2],[0,1]])+self.b)*self.U*self.D+noise+self.mu,tf.float32),self.input_shape)
        def likelihood(self):
                k1  = -tf.reduce_sum(tf.log(self.sigmas2_+eps))/2
                k2  = tf.reduce_sum(tf.reduce_mean(self.p,axis=0)*tf.log(self.pi))
                a1  = -tf.reduce_mean(tf.square(self.input_-self.mu-self.U*self.D*(self.backward(1)+self.b)),0)
                ...
                a3  = tf.reduce_sum(tf.reduce_mean(a30,1),0)*tf.square(self.U_*self.D_)
                a40 = tf.einsum('kr,krd->d',tf.reduce_mean(tf.expand_dims(self.drop_[:,1]*(self.v2_+tf.square(self.m_)),-1)*self.p_,1),tf.square(self.W))
		a4  = -a40*tf.square(self.U_*self.D_)
		k3  = tf.reduce_sum(tf.reduce_mean(self.drop_[:,0],1),0)*tf.log(self.p_drop)+tf.reduce_sum(tf.reduce_mean(self.drop_[:,1],1),0)*tf.log(float32(1)-self.p_drop)
                return k1+k2+k3+tf.reduce_sum((a1+a2+a3+a4)/(2*self.sigmas2_))-self.sparsity_prior*tf.reduce_sum(tf.square(self.W))
        def KL(self):
		if(self.p_drop>0):
                    return self.likelihood()-tf.reduce_sum(tf.reduce_mean(self.p_*tf.log(self.p_+float32(0.0000000000000000000000000000001)),1))+tf.reduce_sum(tf.reduce_mean(tf.log(self.v2_+eps),1))/float32(2)-tf.reduce_sum(tf.reduce_mean(self.drop_*tf.log(self.drop_+float32(0.0000000000000000000000000000001)),2))
		else:
                    return self.likelihood()-tf.reduce_sum(tf.reduce_mean(self.p_*tf.log(self.p_+0.0000000000000000000000000000001),1))+tf.reduce_sum(tf.reduce_mean(tf.log(self.v2_+eps),1))/2
#                                           ----      UPDATES      -----
        def update_v2(self):
                a40     = tf.reduce_sum(tf.square(self.W*myexpand(self.U_*self.D_,[0,1]))*(tf.reshape(self.next_layer.sigmas2_,[self.K,1,1])/tf.reshape(self.sigmas2_,[1,1,self.D_in])),2) # ( K R)
                value   = tf.reshape(self.next_layer.sigmas2_,[self.K,1])/(1+tf.einsum('knr,kr->kn',self.p_*tf.expand_dims(self.drop_[:,1],-1),a40))
                return tf.assign(self.v2_,value)
	def update_U(self):
		back      = (self.backward(1)+self.b)*self.D
		numerator = tf.reduce_mean((self.input_-self.mu)*back,0)
		backk     = tf.einsum('krd,knr->knd',self.W,self.p_)
                denominator = tf.reduce_mean(tf.square(back),0)-tf.reduce_sum(tf.reduce_mean(tf.square(backk*tf.expand_dims(self.m_,-1)),1),0)*tf.square(self.D_)+tf.einsum('kr,krd->d',tf.reduce_mean(tf.expand_dims(self.v2_+tf.square(self.m_),-1)*self.p_,1),tf.square(self.W))*tf.square(self.D_)
		if(self.U_opt=='local'):
                    return tf.assign(self.U_,numerator/denominator)
		elif(self.U_opt=='global'):
                    return tf.assign(self.U_,tf.ones_like(self.U_)*tf.reduce_mean(numerator)/tf.reduce_mean(denominator))
		elif(self.U_opt=='none'):
		    return []
	def update_rho(self):
		return None
        def update_DD(self):
                op_mean = tf.assign(self.mu_,self.batch_norm.center*(1*tf.reduce_mean(self.input_,0)+0.*self.mu_))
                op_std  = tf.assign(self.D_,self.batch_norm.scale*(1*mystd(self.input_,0)+0.*self.D_)+(1-self.batch_norm.scale))
                return tf.group(op_mean,op_std)
        def update_p(self):
                k       = self.k_
		a       = tf.expand_dims(self.input_-(self.backwardmk(self.k_)+self.b)*self.U*self.D-self.mu,1) # (N 1 D)
		b       = tf.expand_dims(self.W[self.k_]*self.U*self.D,0)*tf.reshape(self.m_[k]*self.drop_[k,1],[self.bs,1,1])
		forward = tf.tensordot(tf.square(a-b),1/(2*self.sigmas2_),[[2],[0]]) # ( N R )
                prior   = tf.expand_dims(tf.log(self.pi[k]),0) # (1 R)
		v2      = tf.expand_dims(self.v2_[self.k_],-1)*tf.expand_dims(tf.reduce_sum(tf.square(self.W[self.k_]*self.U*self.D)/(2*self.sigmas2),1),0)
                V       = mysoftmax(-forward+prior-v2)#proj-m2v2+prior)
                return tf.scatter_update(self.p_,[k],[V])
	def update_m(self):
                k       = self.k_
		a       = self.input_-(self.backwardmk(self.k_)+self.b)*self.U*self.D-self.mu # (N D)
                proj    = tf.reduce_sum(tf.tensordot(a*self.next_layer.sigmas2_[k]/self.sigmas2,self.W[k]*self.U*self.D,[[1],[1]])*self.p_[k]*tf.expand_dims(self.drop_[k,1],-1),axis=1) #(N)
                priorm  = (self.next_layer.backwardk(k)+self.next_layer.b_[k])*self.next_layer.U_[k]*self.next_layer.D_[k]+self.next_layer.mu_[k] # ( N )
                a40     = tf.reduce_sum(tf.square(self.W[k]*self.U*self.D)*(self.next_layer.sigmas2_[k]/self.sigmas2),1) # (R)
		v_value = 1+tf.tensordot(self.p_[k]*tf.expand_dims(self.drop_[k,1],-1),a40,[[1],[0]]) #(N)
                return tf.scatter_update(self.m_,[k],[(priorm+proj)/v_value])
        def update_sigma(self):              
                a1  = tf.reduce_mean(tf.square(self.input_-self.mu-(self.backward(1)+self.b)*self.U*self.D),0)
                a2  = tf.reshape(tf.reduce_mean(self.input_layer.v2,0),[self.D_in])
		a4  = tf.einsum('kr,krd->d',tf.reduce_mean(tf.expand_dims((self.v2_+tf.square(self.m_))*self.drop_[:,1],-1)*self.p_,1),tf.square(self.W))*tf.square(self.U_*self.D_) # ( D)
        ...
        def update_Wk(self):
                k = self.k_
		a = self.input_-(self.backwardmk(k)+self.b)*self.U*self.D-self.mu # (N D)
                if(self.nonlinearity is None):
                    numerator   = tf.tensordot(tf.expand_dims(self.m_[k]*self.drop_[k,1],-1)*self.p_[k],a,[[0],[0]])/self.bs # (R D)
                    denominator = tf.reduce_mean(tf.expand_dims((tf.square(self.m_[k])+self.v2_[k])*self.drop_[k,1],-1)*self.p_[k],0) #(R)
                    new_w       = tf.scatter_update(self.W_,[k],[numerator/(self.sparsity_prior/self.U+tf.expand_dims(denominator,-1)*self.U*self.D)])
                elif(self.nonlinearity=='relu'):
                    numerator   = tf.tensordot(self.m_[k]*self.p_[k,:,0],a,[[0],[0]]) # (Din)
                    denominator = tf.reduce_sum((tf.square(self.m_[k])+self.v2_[k])*self.p_[k,:,0])
                    new_w       = tf.scatter_nd_update(self.W_,[[k,0]],[numerator/(self.sparsity_prior/self.U+tf.expand_dims(denominator,-1)*self.U)])
                ...
                return new_w
        def update_b(self):
	       	P     = self.input_-self.backward(1)*self.U*self.D-self.mu
		if(self.b_opt=='local'):
                    return tf.assign(self.b_,tf.reduce_mean(P,axis=[0])/(self.D_*self.U_))
		elif(self.b_opt=='global'):
                    return tf.assign(self.b_,tf.ones_like(self.b_)*tf.reduce_mean(P)*self.D_in/tf.reduce_sum(self.D_*self.U_))
		elif(self.b_opt=='none'):
		    return []
	def update_dropout(self):
                k       = self.k_
                a       = self.input_-(self.backwardmk(self.k_)+self.b)*self.U*self.D-self.mu # (N D)
                proj    = tf.reduce_sum(tf.tensordot(a/self.sigmas2,self.W[k]*self.U*self.D,[[1],[1]])*self.p_[k]*tf.expand_dims(self.m_[k],-1),axis=1) #(N)
                squared0= tf.reduce_sum(tf.square(self.W[k]*self.U*self.D)/self.sigmas2,1) # (R)
...
class ConvPoolLayer:
	def __init__(self,input_layer,K,Ic,Jc,Ir,Jr,R,sparsity_prior = 0,nonlinearity='relu',b='local',sigma='local',U='local',p_drop=0):
                self.nonlinearity      = nonlinearity
		self.sigma_opt         = sigma
		self.b_opt             = b
		self.U_opt             = U
	...
	def large_deconv_(self,u):
        ...
        def helper2(self,k):
            flat_mrho  = myexpand(self.m_[k],[2,3])*self.rho_[k] # (I J Ir Jr N) 
            flat_mrhop = myexpand(flat_mrho,[-2])*myexpand(self.p_[k],[2,3])# (I J Ir Jr R N)#tf.transpose(tf.tensordot(self.p_[k],self.W[k],[[2],[0]]),[3,4,5,2,0,1]) # (I J N Ic Jc C) -> ( N I J)
            return self.W_back(tf.transpose(flat_mrhop,[5,0,1,2,3,4]),self.W[k])/tf.sqrt(self.sigmas2_large_patch) # (N I J ?? ?? C)
#                                           ----  INITIALIZER    ----
        def init_thetaq(self):
	...
	def sample(self,M,K=None,sigma=1):
		...
		noise = sigma*tf.random_normal(self.input_shape)*tf.sqrt(self.sigmas2)
		    ...
		    reverted         = self.revert(tf.reshape(M,[self.bs,self.I,self.J,self.K,1,1])*pool_hot_reshape) # (N I' J' K)
                return self.deconv(sigma_hot_w*myexpand(reverted,[4,5,6]))*self.U+noise+self.b#+tf.expand_dims(self.bb_,0)
        def likelihood(self):
#                back = self.backward()*self.Iin*self.Jin
		# RECONSTRUCTION
                a1 = -tf.reduce_sum(tf.reduce_mean(tf.square(self.input-self.b-self.deconv()*self.U),0)/(2*self.sigmas2))
                a2 = -tf.reduce_sum(tf.reduce_mean(self.input_layer.v2,0)/(2*self.sigmas2))
	...
	def update_dropout(self):
		...
		patches = tf.extract_image_patches((self.input-deconv_without-self.b)*deconv_with/self.sigmas2,
		...
		selected_patches = patches[:,self.i_::self.ratio,self.j_::self.ratio] # (N I J ??) -> (N I'' J'' ??)
                ...
                if(isinstance(self.next_layer,ConvPoolLayer)):
		    b        = self.next_layer.deconv()*self.next_layer.U+self.next_layer.b
		    ...
		    b        = self.next_layer.backward(0)*self.next_layer.reshape_U+self.next_layer.reshape_b
	...
	def update_p(self):
		...
		a     = self.input-deconv_without*self.U-self.b
        ...
        def update_rho(self):
                # Forward Information
                deconv_without = self.deconv(None,1,0)
                patches = tf.reshape(tf.extract_image_patches((self.input-deconv_without*self.U-self.b)*self.U/self.sigmas2,(1,self.Ic,self.Jc,1),(1,1,1,1),(1,1,1,1),"VALID"),
...
#		return []
		...
		m_reverted       = self.revertk(myexpand(self.m[:,:,:,self.k_],[-1,-1])*tf.ones((1,1,1,self.Ir,self.Jr)))[:,:,:,0]*self.rho[:,:,:,self.k_]*self.p[:,:,:,self.k_,self.r_]  # (N I' J')
		    ...
		    deconv          = self.deconv(None,0,1)#mijkr(self.i_,self.j_,self.k_,self.r_) # (N Iin Jin C)
                    reconstruction  = (self.input-deconv*self.U-self.b)*self.U/self.sigmas2 # (N Iin Jin C)
        ...
        def update_b(self):
#		return [None]
	        P = self.input-self.deconv()*self.U
		if(self.b_opt=='local'):
	            return tf.assign(self.b_,tf.reduce_mean(P,axis=[0]))
		elif(self.b_opt=='channel'):
                    return tf.assign(self.b_,tf.ones([self.Iin,self.Jin,self.C])*myexpand(tf.reduce_mean(P/self.sigmas2,axis=[0,1,2])*self.Iin*self.Jin*(1/tf.reduce_sum(1/self.sigmas2_,[0,1])),[0,1]))
		elif(self.b_opt=='global'):
                    return tf.assign(self.b_,tf.fill([self.Iin,self.Jin,self.C],tf.reduce_mean(P)*self.Iin*self.Jin*self.C))
		else:
		    return []
        def update_U(self):
                back      = self.deconv()
                numerator = tf.reduce_mean((self.input-self.b)*back,0)/self.sigmas2_
		...
		if(self.U_opt=='local'):
                    ...
                    return tf.assign(self.U_,tf.ones((self.input_shape[1],self.input_shape[2],self.C))*tf.reduce_mean(numerator)/tf.reduce_mean(denominator))
		else:
		    return []
        def update_sigma(self):
                a1 = tf.reduce_mean(tf.square(self.input-self.b-self.deconv()*self.U),0)
...
class FinalLayer:
        def __init__(self,input_layer,R,sparsity_prior=0,b='local',sigma='local',U='local',batch_norm=BatchNorm(1,1)):
                self.input_layer       = input_layer
		self.sigma_opt         = sigma
		self.batch_norm        = batch_norm
		self.sparsity_prior    = sparsity_prior
		...
		self.W_indices = ones(1)
                ...
                if(len(self.input_shape)>2):
                        ...
                        self.reshape_U        = tf.expand_dims(tf.reshape(self.U_,self.input_shape[1:]),0)
                        self.reshape_D        = tf.expand_dims(tf.reshape(self.D_,self.input_shape[1:]),0)
                        self.reshape_mu       = tf.expand_dims(tf.reshape(self.mu_,self.input_shape[1:]),0)
                        self.reshape_b        = tf.reshape(self.b_,self.input_shape[1:])
                else:
                        self.is_flat = True
                        self.input_  = self.input
		...
		if(self.is_flat):
	                return (tf.tensordot(K,self.W,[[1,2],[0,1]])+self.b)*self.U*self.D+noise+self.mu
		else:
                        return tf.reshape((tf.tensordot(K,self.W,[[1,2],[0,1]])+self.b)*self.U*self.D+noise+self.mu,self.input_shape)
        def likelihood(self):
                ...
                a1  = -tf.reduce_mean(tf.square(self.input_-self.mu-self.U*self.D*(self.backward(1)+self.b)),0)
                a2  = -tf.reshape(tf.reduce_mean(self.input_layer.v2,0),[self.D_in])
                a3  = -tf.reduce_mean(tf.tensordot(self.p_[0],tf.square(self.W[0]),[[1],[0]]),0)*tf.square(self.U_*self.D_) #(N D) -> (D)
                a4  = tf.reduce_mean(tf.square(tf.tensordot(self.p_[0],self.W[0],[[1],[0]])),0)*tf.square(self.U_*self.D_) #(D)
	...
	def update_U(self):
                back = (self.backward(1)+self.b)*self.D
                numerator = tf.reduce_mean((self.input_-self.mu)*back,0)
                denominator = tf.reduce_mean(tf.einsum('rd,nr->nd',tf.square(self.W[0]),self.p_[0]),0)*tf.square(self.D_)
		if(self.U_opt=='local'):
                    return tf.assign(self.U_,numerator/denominator)
		elif(self.U_opt=='global'):
                    return tf.assign(self.U_,tf.ones_like(self.U_)*tf.reduce_mean(numerator)/tf.reduce_mean(denominator))
	...
	def update_v2(self):
        ...
        def update_DD(self):
                ...
                return tf.group(op_mean,op_std)
        def update_p(self):
		proj    = tf.tensordot((self.input_-self.mu-self.b*self.D*self.U)/self.sigmas2,self.W[0]*self.U*self.D,[[1],[1]]) # ( N R)
                prior   = tf.expand_dims(self.pi[0],0)
                m2v2    = -tf.expand_dims(tf.reduce_sum(tf.square(self.W[0]*self.U*self.D)/(2*self.sigmas2),1),0) # ( 1 R )
                V       = mysoftmax(proj+m2v2+tf.log(prior+eps),coeff=0.0)
                return tf.assign(self.p_,tf.expand_dims(V,0))
        def update_sigma(self):
                rec = tf.reduce_mean(tf.square(self.input_-self.mu-(self.backward(1)+self.b)*self.D*self.U),0)
                a1  = tf.reshape(tf.reduce_mean(self.input_layer.v2,0),[self.D_in])
                a3  = tf.reduce_mean(tf.tensordot(self.p_[0],tf.square(self.W[0]*self.U*self.D),[[1],[0]]),0)
                a2  = -tf.reduce_mean(tf.square(tf.tensordot(self.p_[0],self.W[0]*self.U*self.D,[[1],[0]])),0)
                value =  rec+a1+a2+a3
                if(self.sigma_opt=='local'):
                    return tf.assign(self.sigmas2_,value)
		elif(self.sigma_opt=='global'):
        ...
        def update_pi(self):
                ...
                return tf.assign(self.pi,a44/tf.reduce_sum(a44,axis=1,keepdims=True))
        def update_Wk(self):
                rec    = tf.einsum('nd,nr->rd',(self.input_-self.mu-self.b*self.D*self.U),self.p_[0])/self.bs
                KK     = rec/(self.U*self.D*tf.expand_dims(tf.reduce_mean(self.p_[0],0),-1)+self.sparsity_prior)
                return tf.assign(self.W,[KK])
        def update_b(self):
	        P      = self.input_-self.mu-self.backward(1)*self.U*self.D
		if(self.b_opt=='local'):
                    return tf.assign(self.b_,tf.reduce_mean(P,axis=[0])/(self.D_*self.U_))
		elif(self.b_opt=='global'):
		    return tf.assign(self.b_,tf.ones_like(self.b_)*tf.reduce_mean(P)*self.D_in/tf.reduce_sum(self.D_*self.U_))
