...
pp = permutation(N)
XX = x_train[pp]+randn(len(pp),1,28,28)*0.2
YY = y_train[pp]
...
if(MODEL==0):
    ...
    layers1.append(CategoricalLastLayer(layers1[-1],R=NEURONS,sigma=sigmass))
elif(MODEL==1):
    layers1 = [InputLayer(input_shape)]
#    layers1.append(DenseLayer(layers1[-1],K=32,R=2,leakiness=leakiness,sparsity_prior=0.,sigma=sigmass,batch=True,update_b=False))
    layers1.append(ConvLayer(layers1[-1],K=4,Ic=3,Jc=3,R=2,leakiness=0,sparsity_prior=0.,sigma='local'))
    layers1.append(PoolLayer(layers1[-1],Ic=3,Jc=3,sigma='local'))
    layers1.append(CategoricalLastLayer(layers1[-1],R=NEURONS,sigma=sigmass,update_b=False))
elif(MODEL==2):
    layers1 = [InputLayer(input_shape)]
...
print CL
time.sleep(1)
LOSSES  = train_layer_model(model1,rcoeff_schedule=schedule(1.1005,'linear'),alpha_schedule=schedule(0.5,'exp'),CPT=17,random=0,fineloss=0,verbose=1,per_layer=1,mp_opt=0,partial_E=1)
