...
class DenseLayer:
	def __init__(self,input_layer,K,R,sparsity_prior = 0):
...
#                input_layer.next_layer_sigmas2 = self.sigmas2
	...
	def sample(self,M):
		noise = tf.random_normal((self.bs,self.D_in))*tf.sqrt(self.sigmas2[0]) 
...
#                return [new_p]
        def update_pk(self,k):
                ...
                indices = tf.transpose(tf.stack([tf.range(self.bs),tf.constant(k,shape=[self.bs])]))
                V       = tf.exp(tf.clip_by_value((proj+m2v2)/self.sigmas2[0],-37,17))*prior
...
#                KK      = rec2/(self.sparsity_prior+tf.expand_dims(K,-1))
...
#                return []
                ...
                new_p   = tf.scatter_nd_update(self.W,indices,KK)
                return [new_p]#,new_pi]
...
class ConvLayer:
	def __init__(self,input_layer,Ic,Jc,K,R,stride=1,sparsity_prior = 0):
...
#                self.output_shape_= (self.bs,K,R,self.input_shape[-2]/stride,self.input_shape[-1]/stride)
		...
		self.W       = tf.Variable(tf.random_normal((self.K,self.R,self.Ic,self.Jc,self.C))/(self.K*self.Ic*self.Jc))#always D_in last
...
#                return tf.gradients(y,x,tf.expand_dims(self.m,2)*self.p)[0]
	...
	def sample(self,M):
		...
		noise = tf.random_normal(self.input_shape)*tf.sqrt(self.sigmas2[0])
                ...
                return self.deconv(tf.expand_dims(M,-1)*tf.transpose(mp,[1,2,3,0,4]))+noise
        def likelihood(self):
        ...
        def update_pk(self,k):
                ...
                K       = tf.exp(tf.clip_by_value(a1+a2,-20,17))*prior
        ...
        def update_Wk(self,k):
                ...
                indices = tf.transpose(tf.stack([tf.constant(k,shape=[self.R]),tf.range(self.R)]))
                new_w = tf.scatter_nd_update(self.W,indices,up/down)
                return [new_w]
        def update_pi(self):
...
class UnsupFinalLayer:
	...
	def backward(self,flat=1):
        ...
        def update_p(self):
                ...
                V = tf.exp(tf.clip_by_value(a2/self.sigmas2[0],-20,17))*tf.expand_dims(self.pi,0)
                new_p = tf.assign(self.p,V/tf.reduce_sum(V,axis=2,keep_dims=True))
                return [new_p]
        def update_sigma(self):
                rec = -utils.SSE(self.input_,self.backward())
